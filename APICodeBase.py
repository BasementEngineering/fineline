...
class FinancialDataAPI:
    ...
    def _http_request(self, end_point: str, query_string: Dict) -> Dict[str, Any]:
        """
        Make an HTTP request and send the raw response.
        """
        print(query_string)
        print(f"{self.url}{end_point}?{urllib.parse.urlencode(query_string)}")
...
if __name__ == '__main__':
    findata = FinancialDataAPI('Zertificate')
    #sample1 = findata.instrumentBase("ISIN", [row[1]])
    sample2 = findata.endOfDayHistory("ISIN_BC", ["FR0000293698_3230"],"2020-01-01","2024-01-02")
    
    print(json.dumps(sample2, sort_keys=True, indent=4))
    # Access data in each row (list format)
